...
const HeroSection = () => {
  ...
  const prev = () => setCurr((curr) => (curr === 0 ? herobanner.length - 1 : curr - 1));
  const next = () => setCurr((curr) => (curr === herobanner.length - 1 ? 0 : curr + 1));
      ...
      >
        {herobanner.map((link, index) => (
          <div key={index} className="w-full flex-shrink-0 relative">
            <div className="absolute inset-0 flex justify-start items-center text-3xl mt-[27px] flex-col text-center lg:text-black lg:justify-start lg:items-start lg:ml-8 lg:text-6xl lg:text-left lg:w-[1000px]">
              <h1 className="w-[400px] lg:w-[800px]">{link.title}</h1>
              <p className="text-[20px] mt-[8px] w-[308px]">
                {link.para}
              </p>
              <button className="bg-black mt-[8px] pl-[26px] pr-[26px] pt-[5px] pb-[5px] rounded-[36px] text-[15px] text-white lg:pt-3 lg:pb-3 transition ease-in-out delay-150 hover:shadow-2xl hover:-translate-y-1">
                shop now
              </button>
            </div>
            <img
